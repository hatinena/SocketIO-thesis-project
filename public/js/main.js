...
socket.on("userList", (userList) => {
  while (document.getElementById("users").firstChild) {
    document
      ...
      .removeChild(document.getElementById("users").lastChild);
  }
...
function startGame() {
  ...
  //  }
  if (
    document.getElementById("categories").value != "any" &&
    hasGameStarted == false
  ) {
  ...
  $.getJSON(
    "https://opentdb.com/api.php?amount=" +
      document.getElementById("questionsAmount").value +
      category +
      difficulty +
      type +
      "",
...
function addQuestion(i) {
  ...
  try {
    title.innerHTML = htmlDecode(
      i + 1 + "/" + data.length + ". " + data[i].question
    );
  } catch {
    return;
  }
...
document
  .getElementById("questionsAmount")
  .addEventListener("keyup", function (event) {
    if (
      document.getElementById("questionsAmount").value > 50 ||
      document.getElementById("questionsAmount").value < 1
    ) {
      document.getElementById("startbtn").disabled = true;
    } else {
